...
{ 
    ...
    for (int i = 0; i < 10; i++)
    {
    	const clock_t start = ts[i*2];
    	const clock_t stop  = ts[i*2+1];
        printf ("  Block %02d: start: %08u, stop: %08u, diff: %08u\n", i, start, stop, stop - start );
    ...
    {    
        ...
        {
        	const clock_t start = ts[(10*j+i)*2];
    	    const clock_t stop  = ts[(10*j+i)*2+1];
            printf ("  Block %02d: start: %08u, stop: %08u, diff: %08u\n", 10*j+i, start, stop, stop - start );
